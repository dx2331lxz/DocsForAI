...
DocsForAI 是一个针对常见文档框架深度优化的爬虫工具。它能自动识别 VitePress、Docsify、GitBook、Next.js Docs 等多种文档站点，按章节结构提取干净的 Markdown 内容，直接输出给 LLM 或向量数据库使用。
...
| **Next.js Docs** | `_next/` 资源路径 + `.mdx-content` 类               | 通过 `sitemap.xml` 发现全部页面；提取 `.mdx-content` 区域；JSON-LD 面包屑 | tiptap.dev/docs（619 页）      |
